...
import { BUSDBaseTest, VaultAdmin } from "./BUSDBase.t.sol";
...
contract BUSDFactoryPythWrapperTest is BUSDBaseTest {
    BUSDFactoryPythWrapper factoryWrapper;
    bytes[] empty;
    ...
    function setUp() public override {
        ...
        factoryWrapper = new BUSDFactoryPythWrapper(address(factory), address(pyth), address(factoryReader));
        vm.stopPrank();
    }
    ...
    function test_mint() external {
        ...
        vm.expectEmit();
        emit IBUSDFactory.BUSDMinted(address(factoryWrapper), receiver, address(dai), _daiToMint, mintedBusds);
        mintedBusds = factoryWrapper.mint(empty, address(dai), _daiToMint, receiver, false);
    }
    ...
    function testFuzz_mint_WhenBasketModeIsEnabledBecauseOfAllAssetsAreDepegged(uint256 busdsToMintBM) external {
        ...
        uint256 mintedBusds = factoryWrapper.mint(empty, address(dummy), dummyToUseForMint, receiver, true);
    ...
    function test_mint_WhenBasketModeIsEnabledAndAVaultIsPaused() external {
        ...
        factoryWrapper.mint(empty, address(dummy), busdsToMintBM, receiver, true);

        {
            for (uint256 i = 0; i < numAssets; i++) {
                ...
                if (asset == address(usdt)) {
                    ...
                    // Assert Dai amount
                    // see BUSDFactoryReader.sol#L62 for +1 reason
        ...
        {
            // Mint in basket mode:
            uint256 mintedBusds = factoryWrapper.mint(empty, address(usdt), usdtToUseForMint, receiver, true);
    ...
    function testFuzz_redeem_failsWithUnregisteredAsset(uint256 _busdsAmount) external {
        uint256 mintedBusds = _factoryMint(dai, daiBalance, address(this), false);
        _busdsAmount = _bound(_busdsAmount, 1, mintedBusds);

        busd.approve(address(factoryWrapper), _busdsAmount);
        MockUSDT usdtNew = new MockUSDT(); // new unregistered usdt token instance
        vm.expectRevert(abi.encodeWithSelector(IBUSDErrors.AssetNotRegistered.selector, address(usdtNew)));
        factoryWrapper.redeem(empty, address(usdtNew), _busdsAmount, receiver, false);
    }

    function testFuzz_redeem_failWithPausedFactory(uint256 _busdsAmount) external {
        uint256 mintedBusds = _factoryMint(dai, daiBalance, address(this), false);
        _busdsAmount = _bound(_busdsAmount, 1, mintedBusds);
        ...
        factoryWrapper.redeem(empty, address(dai), _busdsAmount, receiver, false);
    }

    function testFuzz_redeem_failsWithInsufficientBusds(uint256 _busdsAmount) external {
        uint256 mintedBusds = _factoryMint(dai, daiBalance, address(this), false);
        _busdsAmount = _bound(_busdsAmount, mintedBusds + 1, type(uint128).max);
    ...
    function testFuzz_redeem_failsWithInsufficientShares(uint256 _daiToMint) external {
        ...
        busd.approve(address(factoryWrapper), (mintedBusds * 3) / 2);
        vm.expectRevert(ERC4626.RedeemMoreThanMax.selector);
        factoryWrapper.redeem(empty, address(dai), (mintedBusds * 3) / 2, address(this), false);
    }
    ...
    function testFuzz_redeem(uint256 _busdsToRedeem) external {
        ...
        vm.prank(receiver);
        busd.approve(address(factoryWrapper), _busdsToRedeem);

        vm.prank(receiver);
        factoryWrapper.redeem(empty, address(dai), _busdsToRedeem, address(this), false);
    ...
    function testFuzz_redeemWithLowerDecimalAsset(uint256 _busdsToRedeem) external {
        ...
        vm.prank(receiver);
        factoryWrapper.redeem(empty, address(usdt), _busdsToRedeem, address(this), false);
    ...
    function testFuzz_redeemWithHigherDecimalAsset(uint256 _busdsToRedeem) external {
        ...
        vm.prank(receiver);
        factoryWrapper.redeem(empty, address(dummy), _busdsToRedeem, address(this), false);
    ...
    function testFuzz_redeem_WhenBasketModeIsEnabledAssetWithWeightZero(uint256 busdsToRedeem) external {
        ...
        busd.approve(address(factoryWrapper), busdsToRedeem);

        factoryWrapper.redeem(empty, address(usdt), busdsToRedeem, address(this), true);
        ...
        external
    {
        ...
        uint256[] memory redemedAmount = factoryWrapper.redeem(empty, asset, busdsToRedeem, address(this), true);
    ...
    function testFuzz_redeem_refundsLeftoverBusdsInBasketMode(uint256 busdsToRedeem) external {
        ...
        uint256 totalBusds = busd.balanceOf(address(this));
        // Use amounts that are likely to cause rounding issues (not divisible by 3)
        busdsToRedeem = _bound(busdsToRedeem, 1e18, totalBusds - 1e11);

        _forceBasketMode();

        uint256 userBusdsBefore = busd.balanceOf(address(this));
        uint256 wrapperBusdsBefore = busd.balanceOf(address(factoryWrapper));
        // wrapper should start with zero busd
        assertEq(wrapperBusdsBefore, 0);

        busd.approve(address(factoryWrapper), busdsToRedeem);
        factoryWrapper.redeem(empty, address(dai), busdsToRedeem, address(this), true);

        uint256 userBusdsAfter = busd.balanceOf(address(this));
        uint256 wrapperBusdsAfter = busd.balanceOf(address(factoryWrapper));

        // The wrapper should have refunded any leftover busd - no busd should remain in wrapper
        assertEq(wrapperBusdsAfter, 0);

        // Calculate expected busd consumed (sum of what was actually burned across all assets)
        uint256[] memory weights = factory.getWeights();
        uint256 totalBurned = 0;
        for (uint256 i = 0; i < weights.length; i++) {
            totalBurned += busdsToRedeem * weights[i] / 1e18;
        }

        // User should have their original balance minus what was actually burned
        // (any rounding leftovers are refunded)
        uint256 expectedUserBusds = userBusdsBefore - totalBurned;
        // user should receive leftover busd from rounding
        assertEq(userBusdsAfter, expectedUserBusds);

        // Verify that rounding can cause some leftover in this case
        assertLe(totalBurned, busdsToRedeem);
    }
    ...
    function test_redeem_failsWhenVaultIsPaused() external {
        ...
        busd.approve(address(factoryWrapper), 50e18);
    ...
    function test_redeem_WhenBasketModeIsEnabledAndAVaultIsPaused() public {
        // Mint busd with a ratio of 33/33/33.
        ...
        uint256[] memory redemedAmounts =
            factoryWrapper.redeem(empty, address(dummy), busdsToRedeem, address(this), true);
    ...
    function test_exceedsRelativeCapWhenRedeemOfAPausedVaultMoveWeightsUponThePausedCollateral() external {
        ...
        busd.approve(address(factoryWrapper), 1e18);
        vm.expectRevert(abi.encodeWithSelector(IBUSDErrors.ExceedRelativeCap.selector));
    ...
    function testFuzz_recapitalize(uint256 daiCollateral, uint256 daiGifted) external {
        ...
        // no busds are minted during recapitalization
    ...
    function test_redeem() external {
        ...
        busd.approve(address(factoryWrapper), mintedBusdsForUsdt);

        vm.expectEmit();
        emit IBUSDFactory.BUSDRedeemed(
            address(factoryWrapper), address(this), address(usdt), redeemedUsdt / 1, mintedBusdsForUsdt
        );
        factoryWrapper.redeem(empty, address(usdt), mintedBusdsForUsdt, address(this), false);
    ...
    function test_redeem_failsWhenReferenceCollateralIsRedeemedAndExceedsRelativeCapOnOtherAssets() external {
        ...
        busd.approve(address(factoryWrapper), 2e18);
        vm.expectRevert(IBUSDErrors.ExceedRelativeCap.selector);
        ...
        returns (uint256 mintedBusds)
    {
        ...
        mintedBusds = factoryWrapper.mint(empty, address(asset), amount, receiver_, expectBasketMode);
    }
