...
contract RewardVaultFactoryTest is POLTest {
    ...
    function test_SetIncentiveTokensCollector_FailIfNotAdmin() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessControl.AccessControlUnauthorizedAccount.selector, address(this), factory.DEFAULT_ADMIN_ROLE()
            )
        );
        factory.setIncentiveTokensCollector(address(this));
    }

    function test_SetIncentiveTokensCollector_FailIfZero() public {
        vm.prank(governance);
        vm.expectRevert(abi.encodeWithSelector(IPOLErrors.ZeroAddress.selector));
        factory.setIncentiveTokensCollector(address(0));
    }

    function testFuzz_SetIncentiveTokensCollector(uint256 collectorInt) public {
        collectorInt = bound(collectorInt, 1, type(uint160).max);
        address collector = address(uint160(collectorInt));
        assertEq(address(0), factory.incentiveTokensCollector());
        vm.prank(governance);
        vm.expectEmit(true, true, true, true);
        emit IRewardVaultFactory.IncentiveTokensCollectorUpdated(collector, address(0));
        factory.setIncentiveTokensCollector(collector);
        assertEq(collector, factory.incentiveTokensCollector());
    }
}
