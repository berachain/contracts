...
/// @title BUSDCollateralVaultCustodyUpgrade
contract BUSDCollateralVaultCustodyUpgrade is Create2Deployer, Test, BUSDAddressBook(ChainType.Mainnet) {
    ...
    function test_Upgrade() public {
        ...
        address newBUSDFactoryImpl = deployWithCreate2(0, type(BUSDFactory).creationCode);
        address newCollateralVaultImpl = deployWithCreate2(0, type(CollateralVault).creationCode);

        vm.startPrank(safeOwner);
        BUSDFactory(_busdAddresses.busdFactory).upgradeToAndCall(newBUSDFactoryImpl, "");
        ...
        vm.prank(safeOwner);
        BUSDFactory(_busdAddresses.busdFactory).setCustodyInfo(usdc, true, usdcCustody);
        ...
        // approve 100 USDC
        ERC20(usdc).approve(_busdAddresses.busdFactory, 100e6);
        BUSDFactory(_busdAddresses.busdFactory).mint(usdc, 10e6, usdcHolder, false);
        ...
        // redeem 5e18 BUSD (5 BUSD)
        BUSDFactory(_busdAddresses.busdFactory).redeem(usdc, 5e18, usdcHolder, false);
        ...
        vm.prank(safeOwner);
        BUSDFactory(_busdAddresses.busdFactory).setCustodyInfo(usdc, false, usdcCustody);
        ...
        // redeem all the minted BUSD of usdcHolder
        BUSDFactory(_busdAddresses.busdFactory).redeem(usdc, 15e18, usdcHolder, false);
