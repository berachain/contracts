...
import { BERACHEF_ADDRESS, DISTRIBUTOR_ADDRESS, REWARD_VAULT_FACTORY_ADDRESS } from "script/pol/POLAddresses.sol";

contract BeraChefUpgradeTest is Create2Deployer, Test {
    ...
    function test_Upgrade() public {
        ...
        vm.prank(safeOwner);
        BeraChef(BERACHEF_ADDRESS).upgradeToAndCall(newBeraChefImpl, bytes(""));

        // check if there is the MAX_COMMISSION_RATE constant
        uint256 maxCommissionRate = BeraChef(BERACHEF_ADDRESS).MAX_COMMISSION_RATE();
    ...
    function test_CommissionRate_PostUpdate() public {
        ...
        uint96 commissionRate = BeraChef(BERACHEF_ADDRESS).getValCommissionOnIncentiveTokens(pubkey);
        ...
        commissionRate = BeraChef(BERACHEF_ADDRESS).getValCommissionOnIncentiveTokens(pubkey);
        ...
        BeraChef(BERACHEF_ADDRESS).queueValCommission(pubkey, newCommissionRate);
        ...
        assertEq(BeraChef(BERACHEF_ADDRESS).getValCommissionOnIncentiveTokens(pubkey), newCommissionRate);
    }
}
