...
contract AddCollateralVaultScript is BaseScript, AddressBook {
    ...
    function addCollateralVault(address collateral) internal {
        ...
        BUSDFactory busdFactory = BUSDFactory(_busdAddresses.busdFactory);

        console2.log("Adding collateral %s", IERC20(collateral).symbol());

        IPriceOracle priceOracle = IPriceOracle(busdFactory.priceOracle());
        // check the price is available
        priceOracle.priceAvailable(collateral);
        ERC4626 vault = busdFactory.createVault(collateral);
        console2.log("Collateral Vault deployed at:", address(vault));
        // Set mint rate to 1:1
        // Only for initial launch
        busdFactory.setMintRate(collateral, 1e18);

        if (isUSDC) {
            // Set peg offset for USDC
            busdFactory.setDepegOffsets(collateral, USDC_PEG_OFFSET, USDC_PEG_OFFSET);
        }
        if (isPyUSD) {
            // Set relative cap for pyUSD
            busdFactory.setRelativeCap(collateral, pyUSD_RELATIVE_CAP);
        }
    }
}
