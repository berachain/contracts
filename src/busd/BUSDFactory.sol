...
/// @notice This is the factory contract for minting and redeeming BUSD.
/// @author Berachain Team
contract BUSDFactory is IBUSDFactory, VaultAdmin {
    ...
    /// @notice The BUSD token contract.
    BUSD public busd;
    ...
    /// @notice Mint rate of BUSD for each asset, 60.18-decimal fixed-point number representation
    mapping(address asset => uint256 rate) public mintRates;
    /// @notice Redemption rate of BUSD for each asset, 60.18-decimal fixed-point number representation
        ...
        initializer
    {
        ...
        if (_busd == address(0)) ZeroAddress.selector.revertWith();
    ...
    /// @notice Set the mint rate of BUSD for an asset.
    ...
    /// @notice Set the redemption rate of BUSD for an asset.
    ...
    function setGlobalCap(uint256 limit) external {
        ...
        // may cause a DoS in the redeem of BUSD
    ...
    /// @dev Create a new ERC4626 vault for a pair of asset - BUSD and register it with VaultAdmin.
        ...
        returns (uint256 busdToMint)
    {
        ...
        if (!basketMode) {
            ...
            busdToMint = _mint(asset, amount, receiver, false);
            ...
            for (uint256 i = 0; i < registeredAssets.length; i++) {
                ...
                busdToMint += _mint(registeredAssets[i], amount, receiver, true);
            }
        }
    }
        ...
        returns (uint256[] memory redeemed)
    {
        ...
        if (!basketMode) {
            ...
            redeemed[index] = _redeem(asset, busdAmount, receiver);
        ...
        redeemed = new uint256[](registeredAssets.length);
        for (uint256 i = 0; i < registeredAssets.length; i++) {
            uint256 amount = busdAmount * weights[i] / 1e18;
    ...
    function _mint(address asset, uint256 amount, address receiver, bool basketMode) internal returns (uint256) {
        ...
        // The factory mints the corresponding amount of BUSD to the receiver
        // with the consideration of the static mint fee.
        (uint256 busdToMint, uint256 feeReceiverFeeShares, uint256 polFeeCollectorFeeShares) =
            _getBUSDMintedFromShares(asset, shares);
        ...
        busd.mint(receiver, busdToMint);

        _checkInvariants(asset);
        emit BUSDMinted(msg.sender, receiver, asset, amount, busdToMint);
        return busdToMint;
    }

    function _redeem(address asset, uint256 busdAmount, address receiver) internal returns (uint256 redeemedAssets) {
        // The function reverts if the sender does not have enough BUSD to burn or
        // the vault does not have enough assets to redeem.
        // The factory burns the corresponding amount of BUSD of the sender
...

        {
            (uint256 sharesForRedeem, uint256 feeReceiverFeeShares, uint256 polFeeCollectorFeeShares) =
                _getSharesRedeemedFromBUSD(asset, busdAmount);
        ...
        emit BUSDRedeemed(msg.sender, receiver, asset, redeemedAssets, busdAmount);
    }

    function _getBUSDMintedFromShares(
        address asset,
        uint256 shares
    )
        internal
        view
        returns (uint256 busdAmount, uint256 feeReceiverFeeShares, uint256 polFeeCollectorFeeShares)
    {
        uint256 mintRate = mintRates[asset];
        busdAmount = shares * mintRate / 1e18;
        uint256 feeShares = shares - busdAmount;
        polFeeCollectorFeeShares = feeShares * polFeeCollectorFeeRate / 1e18;
        feeReceiverFeeShares = feeShares - polFeeCollectorFeeShares;
    }

    function _getSharesRedeemedFromBUSD(
        address asset,
        uint256 busdAmount
    )
        internal
        ...
        returns (uint256 shares, uint256 feeReceiverFeeShares, uint256 polFeeCollectorFeeShares)
    {
        ...
        uint256 feeShares = busdAmount - shares;
