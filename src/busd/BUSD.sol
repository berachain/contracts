...
/// @notice This is the ERC20 token representation of Berachain's native stablecoin, BUSD.
/// @author Berachain Team
/// @dev Uses diamond inheritance for EIP3009 and EIP2612 (both extend ERC20).
contract BUSD is EIP3009, EIP2612, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, IBUSDErrors {
    using Utils for bytes4;

    string private constant NAME = "Bera USD";
    string private constant SYMBOL = "BUSD";

    /// @notice The factory contract that mints and burns BUSD.
    ...
    /// @notice Mint BUSD to the receiver.
    /// @dev Only the factory can call this function.
    /// @param to The receiver address.
    /// @param amount The amount of BUSD to mint.
    function mint(address to, uint256 amount) external onlyFactory {
        _mint(to, amount);
    }

    /// @notice Burn BUSD from an account.
    /// @dev Only the factory can call this function.
    /// @param from The account to burn BUSD from.
    /// @param amount The amount of BUSD to burn.
    ...
    /// @notice Allows to pause transfer of BUSD for a specific wallet
    ...
    /// @notice Allows to pause transfer of BUSD
